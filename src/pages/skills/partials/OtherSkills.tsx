...
const OtherSkills: React.FC<IOtherSkillsProps> = ({logo, skillsGroupArr}) => {
    return (
        <div
            className={'p-6 max-w-sm rounded-lg bg-darkNavy shadow-md text-white flex flex-col items-center justify-around mt-4 w-11/12'}>
            <Anime opacity={[0, 1]} delay={(e: number, i: number) => i * 100} easing={'easeInQuad'}>
                {logo}
                <div className="flex flex-row flex-wrap justify-center">
                    <Anime opacity={[0, 1]} delay={(e: number, i: number) => i * 100} easing={'easeInQuad'}>
                        {skillsGroupArr.map((skill ,index) => (
                            <span
